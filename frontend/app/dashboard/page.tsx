...
const { LineChart, Line, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } = RechartsComponents;
...
// ...existing code...
  ...
  // --- Data Preparation for Analytics Charts ---
  // Time by Project (Line Chart)
  const timeByProject = projects.map(project => {
    const total = timeEntries
      .filter(entry => entry.project === project.name)
      .reduce((sum, entry) => sum + entry.duration, 0);
    return { name: project.name, hours: +(total / 3600).toFixed(2) };
  }).filter(item => item.hours > 0);

  // Productivity Trends (Line Chart, by date)
  const productivityTrends = (() => {
    const byDate = {};
    timeEntries.forEach(entry => {
      if (!byDate[entry.date]) byDate[entry.date] = 0;
      byDate[entry.date] += entry.duration;
    });
    return Object.entries(byDate).map(([date, duration]) => ({
      date,
      hours: +(duration / 3600).toFixed(2)
    }));
  })();

  // Task Completion Analysis (Bar Chart, fake data if not available)
  const taskCompletionData = (() => {
    // If you have real task completion data, use it here
    // For now, use timeEntries as a proxy: count entries per day
    const byDate = {};
    timeEntries.forEach(entry => {
      if (!byDate[entry.date]) byDate[entry.date] = 0;
      byDate[entry.date] += 1;
  ...
  // Time Tracking Overview (Line Chart, hours per day)
  ...
  // Pomodoro Sessions (Bar Chart, sessions per day)
  const pomodoroSessionsBar = (() => {
    const byDate = {};
  ...
  // Dynamically import Recharts components on client
          ...
          <div className="flex-1 p-6">
            <div className="w-[90%] mx-auto space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                >
                  {Recharts && React.createElement(
                    Recharts.ResponsiveContainer,
                    { width: "100%", height: 250 },
                    React.createElement(
                      Recharts.LineChart,
                      { data: timeByProject, margin: { top: 5, right: 20, left: 10, bottom: 5 } },
                      [
                        React.createElement(Recharts.CartesianGrid, { strokeDasharray: "3 3", key: "cg" }),
                        React.createElement(Recharts.XAxis, { dataKey: "name", key: "x" }),
                        React.createElement(Recharts.YAxis, { label: { value: 'Hours', angle: -90, position: 'insideLeft' }, key: "y" }),
                        React.createElement(Recharts.Tooltip, { key: "t" }),
                        React.createElement(Recharts.Legend, { key: "l" }),
                        React.createElement(Recharts.Line, { type: "monotone", dataKey: "hours", stroke: "#8884d8", activeDot: { r: 8 }, key: "line" })
                      ]
                    )
                  )}
                </ChartWrapper>
                ...
                >
                  {Recharts && React.createElement(
                    ...
                    React.createElement(
                      Recharts.LineChart,
                      { data: productivityTrends, margin: { top: 5, right: 20, left: 10, bottom: 5 } },
                      [
                        React.createElement(Recharts.CartesianGrid, { strokeDasharray: "3 3" }),
                        React.createElement(Recharts.XAxis, { dataKey: "date" }),
                        React.createElement(Recharts.YAxis, { label: { value: 'Hours', angle: -90, position: 'insideLeft' } }),
                        React.createElement(Recharts.Tooltip),
                        React.createElement(Recharts.Legend),
                        React.createElement(Recharts.Line, { type: "monotone", dataKey: "hours", stroke: "#82ca9d", activeDot: { r: 8 } })
                      ]
                    )
                  )}
                </ChartWrapper>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                >
                  {Recharts && React.createElement(
                    ...
                    React.createElement(
                      Recharts.BarChart,
                      { data: taskCompletionData, margin: { top: 5, right: 20, left: 10, bottom: 5 } },
                      [
                        React.createElement(Recharts.CartesianGrid, { strokeDasharray: "3 3" }),
                        React.createElement(Recharts.XAxis, { dataKey: "date" }),
                        React.createElement(Recharts.YAxis, { label: { value: 'Tasks', angle: -90, position: 'insideLeft' } }),
                        React.createElement(Recharts.Tooltip),
                        React.createElement(Recharts.Legend),
                        React.createElement(Recharts.Bar, { dataKey: "tasks", fill: "#8884d8" })
                      ]
                    )
                  )}
                </ChartWrapper>
                ...
                >
                  {Recharts && React.createElement(
                    ...
                    React.createElement(
                      Recharts.LineChart,
                      { data: timeTrackingOverview, margin: { top: 5, right: 20, left: 10, bottom: 5 } },
                      [
                        React.createElement(Recharts.CartesianGrid, { strokeDasharray: "3 3" }),
                        React.createElement(Recharts.XAxis, { dataKey: "date" }),
                        React.createElement(Recharts.YAxis, { label: { value: 'Hours', angle: -90, position: 'insideLeft' } }),
                        React.createElement(Recharts.Tooltip),
                        React.createElement(Recharts.Legend),
                        React.createElement(Recharts.Line, { type: "monotone", dataKey: "hours", stroke: "#ff7300", activeDot: { r: 8 } })
                      ]
                    )
                  )}
                </ChartWrapper>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                >
                  {Recharts && React.createElement(
                    ...
                    React.createElement(
                      Recharts.BarChart,
                      { data: pomodoroSessionsBar, margin: { top: 5, right: 20, left: 10, bottom: 5 } },
                      [
                        React.createElement(Recharts.CartesianGrid, { strokeDasharray: "3 3" }),
                        React.createElement(Recharts.XAxis, { dataKey: "date" }),
                        React.createElement(Recharts.YAxis, { label: { value: 'Sessions', angle: -90, position: 'insideLeft' } }),
                        React.createElement(Recharts.Tooltip),
                        React.createElement(Recharts.Legend),
                        React.createElement(Recharts.Bar, { dataKey: "sessions", fill: "#82ca9d" })
                      ]
                    )
                  )}
                </ChartWrapper>
